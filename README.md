...
This technique also brought some issues and that's 'slow insertion', since nodes are stored inside a vector, insert nodes frequently can be very slow (check input and output time comparison, it's scary).

But, there is also a technique called `document_tree_batch_inserter` which uses `std::forward_list` to improve insertion speed. 
`document_tree_batch_inserter` is basically just a sub tree but without erase and searching methods.
...
jxxson::document_tree_parser<>{tree}(std::istreambuf_iterator<char>(in_json), std::istreambuf_iterator<char>());
